...
module.exports = {
     ...
     * @param  {object}   res  Response object
     * @param  {Function} next Callback function to move on to the next middleware
     * @return {object}        Populates res.locals with attendance document
     */
    getOne: (req, res, next) => {
    ...
    startAttendance: (req, res, next) => {
        log.info('Module - startAttendance Attendance');
        ctrls.mongodb.findById(models.attendances, req.params.id, (err, attendance) => {
            if (err) {
                let err = new Error('Failed getting attendance: ' + req.params.id);
                err.status = 500;
                next(err);
            ...
            ctrls.mongodb.save(attendance, (err, _result) => {
                if (err) {
                    let err = new Error('Failed Activating Attendance!');
                    err.status = 500;
                    next(err);
                    return;
                }

                log.info('Succesfully activated attendance [' + req.params.id + ']');
                next();
            });
        });
    },
    ...
    stopAttendance: (req, res, next) => {
        log.info('Module - stopAttendance Attendance');
        ctrls.mongodb.findById(models.attendances, req.params.id, (err, attendance) => {
            if (err) {
                let err = new Error('Failed getting attendance: ' + req.params.id);
                err.status = 500;
            ...
            ctrls.mongodb.save(attendance, (err, _result) => {
                if (err) {
                    let err = new Error('Failed stopping Attendance!');
                    err.status = 500;
                    next(err);
                    return;
                }

                log.info('Succesfully stopped attendance [' + req.params.id + ']');
                next();
            });
        });
    }
